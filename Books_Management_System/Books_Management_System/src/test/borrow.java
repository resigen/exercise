...
import entity.booklist;
	...
	public class borrow extends JFrame {
		...
		public borrow()
		{
			...
			borrowBtn.setBounds(10, 160,70,30);
			...
			this.getContentPane().add(borrowBtn);
...
//			表格中---内容 （二维数组） 标题（一维数组）
			...
			for(int i=0;i<bookList.size();i++)
			{
				...
				System.out.println(u.getstates());
			}
		...
		public void borrows()
		{
			...
			if(row==-1)
			{
				JOptionPane.showMessageDialog(this,  "请选择您要借阅的书籍","错误消息", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			String flag2="否",flag1;
			flag1=jt.getValueAt(row, 6).toString();
			if(flag1.equals(flag2)) {
				
				booklistDAO dao=new booklistDAO();
				
				boolean flag=dao.borrowed(id);
				
				if(flag)
				{
					JOptionPane.showMessageDialog(this,"外借成功", "删除",  JOptionPane.ERROR_MESSAGE);
					query();
				}
				else
				{
					JOptionPane.showMessageDialog(this,"外借失败",  "删除", JOptionPane.ERROR_MESSAGE);
					query();
				}
			}
			else
